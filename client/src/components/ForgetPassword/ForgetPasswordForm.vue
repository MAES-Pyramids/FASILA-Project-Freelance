    ...
    <div class="col-lg-5 col-md-8 m-auto">
        <div class="card shadow py-1">
            <div class="card-body">
                ...
                <form class="d-flex flex-column justify-content-center align-items-center gap-2" @submit.prevent="sendOTP">
                    ...
                    <n-button type="primary" class="w-100" @click="sendOTP" block secondary strong v-else>
                        إرســـــــال
                    </n-button>
                </form>
                ...
                await axios.get(`/students/public?mobileNumber=2${this.state.form.phone}`)
                    .then((response) => {
                        if (response.status == 200) {
                            ...
                            socket.on('otp-passReset', (msg) => {
                                if (msg.status == true) {
                                    const modalElement = document.getElementById('modalId');
                                    const existingModal = bootstrapBundleMin.Modal.getInstance(modalElement);
                                    existingModal.hide();
                                    this.$router.push('/resetPassword');
                                    useToast().success(`سيتم تحويلك لصفحة إعادة تعيين كلمة المرور`);
                                    this.$store.state.auth.token = msg.message;
                                } else {
